...
The question ``what are you taking this semester?'' may possibly be the most common phrase uttered in campus smalltalk within the first few weeks of a semester (as well as its analogue ``what are you taking \emph{next} semester?'' in the last half of a semester).
...
\begin{figure}[H]
  ...
  \begin{tabular}{c c}
    ...
    \begin{subfigure}[w]{7cm}
      \centering
      \includegraphics[width=6.5cm]{images/cdcs/social}
      \caption{Sharing a static schedule image generated by Better CDCS on Facebook} \label{fig:cdcs-social}
    \end{subfigure}

    \hspace{10pt}

    \begin{subfigure}[w]{7.1cm}
      \centering
      \includegraphics[width=6.6cm]{images/skedge/facebook}
...
\subsection{Limitations of the Facebook model, remedied by Skedge}
  ...
  \begin{enumerate}
    ...
    \item typically only occurs for the \emph{current} semester. Under the Facebook model, it can be very difficult, nigh impossible, for students seeking advice, opinions, or even academic help for a specific class to find others who had taken that class before.
  \end{enumerate}
...
The \emph{Skedge Social} platform aims to leverage existing social media paradigms (such as the \emph{news feed}, \emph{likes}, and \emph{requests}) to more seamlessly give users answers to the questions alluded to earlier---``What are my friends taking?'' and ``What do my friends recommend?''. 

Logging into Skedge Social is very simple---if the user is already logged into Facebook, it just requires one button of authorization (see Figure \ref{fig:sk-social-login}).
  ...
  Immediately after logging in, the user will see the ``schedule feed,'' the list of the user's peers' current schedules. This takes from a familiar paradigm of the \emph{feed} in social networks, which is routinely scrolled for updates. For each schedule in the feed, the user can easily visualize their own schedule side-by-side (using with the same ``overlay'' mechanic explained earlier), access the Facebook profile of the schedule's owner, and see the list of that users' course like.
    ...
    \begin{figure}
      ...
      \caption{Skedge Social's schedule feed (the default of four tabs) for a user who has integrated their Skedge account with Facebook. The user can toggle between a peer's schedule and the list of their \emph{liked} courses.} \label{fig:sk-social}
    \end{figure}
  
  \subsubsection{Search result integration}

  Skedge Social also integrates into course search results, showing users on the platform their Facebook friends who are taking, have taken, or \emph{liked} each particular course (see Figure \ref{fig:sk-social-course}). This has several advantages:
