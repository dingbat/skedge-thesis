...
\subsection{Direct search (required courses)}
  ...
  The difference between these results and those in Figure \ref{fig:navs-combined} is subtle but logical. Users searching for a specific course should have an increased proportion of 1-navigation adds, as the graph shows, because the use-case is ``\emph{search for course}'' followed by ``\emph{add course}'', perhaps occasionally taking an extra try to account for typos, etc.

  But given this reasoning, why would 0-navigation adds still be so common in direct searches? Further analysis on sections added from 0 navigations (from direct search) shows that 64\% of those sections were in fact \emph{subsections}. As was illustrated in the example at the beginning of this section, this demonstrates the very common use-case of adding labs, workshops, etc. of a course directly after adding the main section when directly searching.

  This hypothesis is further confirmed by doing the same analysis for ${navs}=1$ and ${navs}=2$, when the inverse behavior would be expected (it makes sense for subsections which were added seperately from their main sections to be more rare). Only 11\% of sections that were added from a direct search after 1 navigation were subsections, and that number drops to 8\% for 2 navigations.

\subsection{Browse (elective courses)}
...
\subsection{Peer-guided (recommended courses)}
